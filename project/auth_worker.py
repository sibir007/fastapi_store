...
async def auth_handler(auth_data: AuthUserData, logger: Logger) -> UserBrokerResult:
    ...
    return UserBrokerResult(result=user)
...
@broker.subscriber(list="register")
async def user_register_handler(user_reg: SUserIn, logger: Logger) -> UserBrokerResult:
    ...
    return UserBrokerResult(result=user_out)
